...
def daloop_inference(era5_dir,
    ...
    if da_model in ["vit"]:
        ...
        assim_model = FdvarCycleGANAssimModule.load_from_checkpoint(f"{ckpt_dir}/{da_model}_assim_{NAME_TO_VAR[var]}{var[-3:]}_xb{init_time}h.ckpt",
                                                                    pred_ckpt=f"{ckpt_dir}/afnonet_{NAME_TO_VAR[var]}{var[-3:]}_0714.ckpt")
        assim_net = assim_model.g_A2B.to(device).eval()
    ...
    xb_iter.to_netcdf(f'{output_dir}/xb_iter_{da_model.lower()}_obspartial{obs_partial}_xb{init_time}_retrain.nc')
    print(f'{output_dir}/xb_iter_{da_model.lower()}_obspartial{obs_partial}_xb{init_time}_retrain.nc saved!')
    fc_iter.to_netcdf(f'{output_dir}/fc_iter_{da_model.lower()}_obspartial{obs_partial}_xb{init_time}_retrain.nc')
    print(f'{output_dir}/fc_iter_{da_model.lower()}_obspartial{obs_partial}_xb{init_time}_retrain.nc saved!')
    xr_rmse.to_netcdf(f'{output_dir}/rmse_{da_model.lower()}_obspartial{obs_partial}_xb{init_time}_retrain.nc')
    print(f'{output_dir}/rmse_{da_model.lower()}_obspartial{obs_partial}_xb{init_time}_retrain.nc saved!')
    xr_acc.to_netcdf(f'{output_dir}/acc_{da_model.lower()}_obspartial{obs_partial}_xb{init_time}_retrain.nc')
    print(f'{output_dir}/acc_{da_model.lower()}_obspartial{obs_partial}_xb{init_time}_retrain.nc saved!')
    xr_mae.to_netcdf(f'{output_dir}/mae_{da_model.lower()}_obspartial{obs_partial}_xb{init_time}_retrain.nc')
    print(f'{output_dir}/mae_{da_model.lower()}_obspartial{obs_partial}_xb{init_time}_retrain.nc saved!')
    np.save(f'{output_dir}/da_time_{da_model.lower()}_obspartial{obs_partial}_xb{init_time}_retrain.npy', times4da)
