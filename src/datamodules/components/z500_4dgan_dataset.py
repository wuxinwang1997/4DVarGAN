...
class Z500_DA_Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        y = self.input_y[idx%self.input_xb.shape[0] : idx%self.input_xb.shape[0] +2 * self.pred_len + 1 : 2]
        # random erase some observation points
        mask_y = self.mask_y[idx % self.input_xb.shape[0] : idx%self.input_xb.shape[0] +2 * self.pred_len + 1 : 2]
        if self.random_erase:
            for i in range(self.pred_len):
                mask_tmp = np.ndarray.flatten(mask_y[i])
                nonzero_mask_y = np.nonzero(mask_tmp)[0]
                if self.mode == 'train':
                    erase_mask_y = np.random.choice(nonzero_mask_y, np.random.randint(low=0, high=len(nonzero_mask_y)))
                    mask_tmp[erase_mask_y] = 0
                else:
            ...
            mask_y[i] = np.reshape(mask_tmp, mask_y[i].shape)

        gt = self.labels[idx%self.input_xb.shape[0] : idx%self.input_xb.shape[0]+2*self.pred_len+1 : 2]
