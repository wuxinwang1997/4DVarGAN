...
class Discriminator(nn.Module):
    ...
    def __init__(self,
        ...
        self.layers = [conv, nn.InstanceNorm2d(out_channels), nn.LeakyReLU(0.2, True)]

        for idx in range(1, num_layers):
            conv = nn.Conv2d(out_channels * in_f, out_channels * out_f, kernel_size=4, stride=2, padding=1)
            self.layers += [conv, nn.InstanceNorm2d(out_channels * out_f), nn.LeakyReLU(0.2, inplace=True)]
            in_f = out_f
            out_f *= 2
        ...
        mean = nn.AdaptiveAvgPool2d(1)
        conv = nn.Conv2d(out_channels * out_f, out_channels=out_channels, kernel_size=1, stride=1, padding=0)
        self.layers += [mean, conv, nn.LeakyReLU(0.2, inplace=True)]
        conv = nn.Conv2d(out_channels, out_channels=1, kernel_size=1, stride=1, padding=0)
        self.layers += [conv]
...
class SNDiscriminator(nn.Module):
    ...
    def __init__(self,
        ...
        for idx in range(1, num_layers):
            conv = spectral_norm(nn.Conv2d(out_channels * in_f, out_channels * out_f, kernel_size=4, stride=2, padding=1))
            self.layers += [conv, nn.InstanceNorm2d(out_channels * out_f), nn.LeakyReLU(0.2, inplace=True)]
            in_f = out_f
            out_f *= 2

        out_f = min(2 ** num_layers, 8)
        conv = spectral_norm(nn.Conv2d(out_channels * in_f, out_channels * out_f, kernel_size=4, stride=1, padding=1))
        self.layers += [conv, nn.InstanceNorm2d(out_channels * out_f), nn.LeakyReLU(0.2, inplace=True)]

        mean = nn.AdaptiveAvgPool2d(1)
        conv = spectral_norm(nn.Conv2d(out_channels * out_f, out_channels=1, kernel_size=1, stride=1, padding=0))
        self.layers += [mean, conv]
        # conv = nn.Conv2d(out_channels * out_f, out_channels=1, kernel_size=4, stride=1, padding=1)
        # self.layers += [conv]
