...
def compute_weighted_rmse(fc, true, lat):
    """
    ...
    """
    error = fc - true
    weights_lat = torch.unsqueeze(torch.unsqueeze(torch.cos(torch.deg2rad(lat)), dim=-1), dim=0)
    weights_lat /= torch.mean(weights_lat)
    rmse = torch.sqrt(torch.mean((error)**2 * weights_lat))
    return rmse

def compute_weighted_acc(da_fc, da_true, clim, lat):
    """
...
def compute_weighted_mae(da_fc, da_true, lat):
    """
    ...
    weights_lat = torch.unsqueeze(torch.unsqueeze(torch.cos(torch.deg2rad(lat)), dim=-1), dim=0)
    weights_lat /= torch.mean(weights_lat)
    mae = torch.mean(torch.abs(error) * weights_lat)
    return mae
...
#     acc = (
#             (w * fa_prime * a_prime).sum() /
#             np.sqrt(
#                 (w * fa_prime ** 2).sum() * (w * a_prime ** 2).sum()
#             )
#     )
#     return acc
