...
class Z500_DAMO_Dataset(Dataset):
    def __init__(self, 
                 data_dir, 
                 xb_dir, 
                 init_time,
        ...
        if self.correct:
            self.input_xerr = self.input_xerr[:-1]
        self.input_xb = self.input_xb[:-1]
    ...
    def __getitem__(self, idx):
        ...
        y = self.input_y[idx % self.input_xb.shape[0]:idx % self.input_xb.shape[0]+2]
        # random erase some observation points
        mask_y = self.mask_y[idx % self.input_xb.shape[0]:idx % self.input_xb.shape[0]+2]
